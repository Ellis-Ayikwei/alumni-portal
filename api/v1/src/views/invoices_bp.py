...
import os
from click import FLOAT, group
from colorama import Fore, Style
from werkzeug.utils import secure_filename
from flask import Flask, current_app, jsonify, make_response, request, abort, send_file, send_from_directory
...
def get_users_invoices(user_id):
    ...
    users_invoices = [invoice.to_dict() for invoice in invoices if invoice.billed_user_id == user_id]
...
def create_invoice():
    ...
    try:
        print(form_data)
        new_invoice = Invoice(
            invoice_number=form_data["invoice_number"] if "invoice_number" in form_data else None,
...
def update_invoice(invoice_id):
    ...
    ignore = ['id', 'created_at', 'updated_at', '__class__', 'invoice_number', 'group', 'billed_user']
