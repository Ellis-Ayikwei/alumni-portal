...
def create_payment():
    ...
    required_fields = ['amount', 'payment_date', 'payer_id', 'group_id', 'payment_method_id']
...
#     """Serve a previously uploaded file for viewing"""
...
#     # Debugging logs
#     current_app.logger.info(f"Trying to serve file: {file_path}")

#     if not os.path.exists(file_path):
#         current_app.logger.error(f"File not found: {file_path}")
#         return jsonify({"error": "File not found"}), 404

#     # Use send_file to stream the file directly to the browser
